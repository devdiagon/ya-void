    ...
    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <WarehouseIcon size={20} className="text-sky-500" />
              ...
              {requesters.map((requester) => (
                ...
                >
                  <div className="flex items-center gap-3 text-gray-700">
                    <User2Icon size={18} className="text-cyan-600" />
                    <span className="text-2lg">{requester.name}</span>
                  </div>
                  <RequesterOptions
                    requester={requester}
                    onEdit={() => updateRequesterModal.open(requester)}
                    onRemove={() => removeRequesterFromArea(requester.id)}
                  />
                </div>
              ))}
            </div>
